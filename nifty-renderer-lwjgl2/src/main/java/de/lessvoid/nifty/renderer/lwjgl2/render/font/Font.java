package de.lessvoid.nifty.renderer.lwjgl2.render.font;
...
import de.lessvoid.nifty.renderer.lwjgl2.render.LwjglRenderImage2;
...
public class Font {
  ...
  private int listId;
  private LwjglRenderImage2[] textures;
  ...
  public boolean init(final String filename) {
    ...
    textures = new LwjglRenderImage2[font.getTextures().length];
    for (int i = 0; i < font.getTextures().length; i++) {
      //textures[i] = new LwjglRenderImage2(extractPath(filename) + font.getTextures()[i], true, resourceLoader);
    }
